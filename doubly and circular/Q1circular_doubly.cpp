...
public:
    int data;
    node *next; // pointer to the next node in CLL(doubly)
    node *prev; // pointer to the previous node in CLL(doubly)
...
node *takeinput()
{
    ...
    cin >> data;
    node *head = NULL;
    // tail bna lo ya hr baar traverse krlo last node tak par last node toh chahie hogi(temp->next!=head)
    while (data != -1)
    {
